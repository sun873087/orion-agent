...
上一輪一整套 LLM-enhanced UX(title / banner / tool / error / follow-up / 摘要 /
草稿 / cost ledger / soul.md / `?` cheat sheet)已併入。這輪換主軸:
**透明度**(讓 user 看到 Orion 知道什麼 / 送什麼出去 / 不該看到什麼)+
**跨 session recall**(soul.md 是長期人格,recap 是短期「上次聊到哪」)。設計
原則承襲上輪:0 預設成本 / silent fallback / injection 防護 / 沿用單一 cheap
model channel。**已完成**:A1 audit 追溯 / A2 wire payload audit / B2 跨對話
搜尋 Tool(對齊 Anthropic `conversation_search` 模式,SQLite FTS5,zero token
cost)/ C1 訊息品質 👍 👎(👎 在 SQL 層 hard-排除,不靠 prompt 控制)。
**接下來**:A3 敏感資訊偵測 / B1 recap chip / B3 對話 export 等。

→ [`trust-and-recall.md`](./trust-and-recall.md)
...
- [`trust-and-recall.md`](./trust-and-recall.md) — 下一輪對話 polish:透明度三件套 + 跨 session recall
