...
title 自動摘要、banner 翻譯、tool 解釋、follow-up 建議句已完成,接下來想做 tool
error 解釋、vague prompt 補問、訊息摘要、草稿保存、`?` cheat sheet、Cmd+K
command bar、成本 breakdown、user profile 學習等。**設計原則**:0 預設成本、
LLM 失敗 silent fallback、prompt injection 防護、沿用單一 cheap model channel。
