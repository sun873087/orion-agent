...
**現況**:Anthropic + OpenAI + Ollama + **OpenRouter**(gateway, 100+ models 從各 vendor)+ **Google Gemini**(OpenAI-compat 端點),SSE streaming,proxy 透傳。

**方向**:

- **更多 direct provider**:Cohere、Mistral、Groq、xAI Grok、自架 vLLM(走 OpenAI-compat)— 透過 OpenRouter 已能用,但 direct 整合可省 markup
...
上一輪 trust-and-recall 已併入(A1 audit / A2 wire payload / B2 ConversationSearch /
C1 thumbs feedback);再往「LLM 主動」推(C2 對話 → todo / recap chip / 自動
export 提示)user 都退回了。教訓:**不做 LLM 主動建議路線 / 不另開 panel 複製
既有功能 / 不依賴 LLM 行為合作**。這輪重新從 user 真實痛點出發,主軸偏 **日常
navigation / cost 控制 / 小事一鍵搞定**。候選:A. Cmd+K 全局 command palette /
B. Cost 防爆送出前估算 / C. Message bookmark / D. 換 model 重試並排比對 + 7 個
polish 池項目。

→ [`everyday-ux.md`](./everyday-ux.md)
...
- [`everyday-ux.md`](./everyday-ux.md) — 下一輪日常使用優化:Cmd+K / cost 防爆 / bookmark / 換 model 重試 + polish 池
