...
| Feature | 為什麼 |
|---|---|
| ~~**PyInstaller 打包 + electron-builder cross-platform**(.app / .exe / .AppImage)~~ ✅ **Phase 31-W macOS 完成** — `pnpm dist:mac` 一鍵跑(`build-sidecar.sh` 跑 pyinstaller.spec 產 90 MB binary → vite build renderer → electron-builder 包 DMG)。Binary 跟 extraResources 走 `Contents/Resources/sidecar/`,production `main.ts:packagedSidecarPath()` 偵測 isPackaged 切換 spawn path。arm64 / x64 DMG 都 build 出。Win/Linux 設定齊全但要在對應 OS 跑(沒 CI matrix);notarize 留下一個 phase。詳見 [`docs/guides/build.md`](../guides/build.md) | 目前只能 `pnpm dev`,沒法給 non-dev user 用 |
