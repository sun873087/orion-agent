...
| ~~**檔案直接拖入對話**(drop file → 自動 inject content + path 進 system context)~~ ✅ **Phase 31-N 完成** — 文字 / code 檔拖入自動讀 content + path inline 進 prompt,Electron 32+ webUtils 拿絕對路徑;圖片走原 attachment 路徑;500 KB 上限 | 目前要 `/add-files` 或叫 LLM Read,user 手感差 |
