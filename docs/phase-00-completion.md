...
- [x] **完整 query loop**:user prompt → model → 若 ToolUseBlock 則執行工具 → 把 ToolResultBlock 回填 → 模型再請求 → 直到 `stop_reason=end_turn` 且無 tool_use 〈Phase 1 ✓〉
- [x] **不要只看 stop_reason** 判斷有沒有 tool_use(見上方坑 #2)〈Phase 1 ✓〉
- [x] **平行工具執行**:Tool.is_concurrency_safe 為 True 的可同時跑(對應 TS Claude Code mh1)〈Phase 1 partition_tool_calls + Phase 1b streaming executor ✓〉
- [x] **Tool 結果序列化**:超過 `max_result_size_chars` 要持久化 + 回 model 摘要 〈Phase 2 第 2 層 maybe_persist_large_tool_result ✓〉
- [ ] **Abort 機制**:用 `AgentContext.abort_event` 真實中斷 stream 〈**部分**:turn 邊界 + BashTool 監看已做;stream 中途即時中止留 Phase 3/7 anyio cancel scope〉

### Phase 0 留下的小債

- [x] 查 `uv pip install -e .` 偶爾沒裝 source 的 root cause(坑 #3)〈Phase 2b ✓ — 根因是 iCloud Drive Sync,Makefile + TROUBLESHOOTING.md 修〉
- [x] `tests/integration/` 加實際 case(需 API key)〈Phase 1 ✓ — test_anthropic_loop.py + test_openai_loop.py〉
