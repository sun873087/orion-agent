# Phase 29:Auth user_id ↔ DB FK 對齊

## 速覽

- **時程**:約 1 天
- **前置 Phase**:Phase 6(FastAPI auth)、Phase 7(register/login)、Phase 14(user_settings)
- **觸發來源**:audit 時發現 schema FK 指 `users.id`(UUID)但 JWT `sub` 是 username 字串,SQLite FK 預設 off 沒爆,但 Postgres 上會死
- **主要交付物**:
