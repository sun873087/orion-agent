...
- [x] StreamingToolExecutor 接到 query_loop 主路徑(目前只有 unit test 覆蓋)〈Phase 1b ✓〉
- [x] BashTool 觸發 sibling_abort,中止並行兄弟工具 〈Phase 1b ✓〉
- [x] Conversation.stats 累積 input_tokens / output_tokens(目前都是 0)〈Phase 2 順手 ✓〉
- [x] Glob fallback 對大目錄的記憶體控制(目前一次性 list)〈Phase 2b heapq ✓〉

### Phase 2 才該做

- [x] 工具結果三層持久化(transcript / disk overflow)〈Phase 2 ✓〉
- [x] resume 機制 〈Phase 2 ✓〉
- [x] tool_use_id ↔ tool_result 配對的持久化校驗 〈Phase 2b validate_and_repair_messages ✓ — 強校驗 + auto-repair〉

### 觀察到的後續優化機會(nice-to-have,非 TODO)

- [ ] query_loop 的 `final_messages` 隨 turn 滾雪球 → 留 Phase 3 compaction 處理
- [ ] AgentTool 子 agent max_concurrent_sub_agents limit(避免一次開太多 LLM 連線)
- [ ] WebFetchTool caching(同 URL 重複 fetch 浪費)
