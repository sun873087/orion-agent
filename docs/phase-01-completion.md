...
**完成日期**:2026-05-07
**Spec doc**:`docs/phases/01-agent-loop.md`
...
| 後續 phase 完成的延伸工作 | 哪裡完成 |
|---|---|
| StreamingToolExecutor 接 query_loop 主路徑 | Phase 1b |
| BashTool sibling_abort | Phase 1b |
| Conversation.stats input/output token 累積 | Phase 2 順手 |
| Glob fallback 大目錄記憶體控制 | Phase 2b(heapq) |
| 工具結果三層持久化、resume、tool_use_id 配對校驗 | Phase 2 / Phase 2b |

## 衍生的新 phase plan

實作中觀察到的延後 / nice-to-have,均升級為獨立 phase plan:

- AgentTool 子 agent 全域並發上限 → [`docs/phases/plan/17-agenttool-concurrency-limit.md`](phases/plan/17-agenttool-concurrency-limit.md)
- WebFetchTool URL caching → [`docs/phases/plan/18-webfetch-cache.md`](phases/plan/18-webfetch-cache.md)

`final_messages` 隨 turn 滾雪球本就在 Phase 3 spec(memory / compaction)範圍內,不另開。
