...
`packages/orion-sdk/src/orion_sdk/skills/bundled/` 預裝 12 個(Phase 31-K 為止):
...
| `agent` | 平行 spawn sub-agents — 教 LLM 如何用 `Agent` 工具(decide / compose task / fan out / synthesize);對應 `/agent <task>` | true |
