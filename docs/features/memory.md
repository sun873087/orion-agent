...
orion-agent 把對話結束後 LLM 認為值得記的事實,以 markdown 檔形式存到 `~/.orion/users/<id>/memory/`。每輪對話開始時,relevance ranker 選出 top-N 注入 system prompt — 等同 agent 的長期記憶。

模組位置:`packages/orion-sdk/src/orion_sdk/memory/`(extract、scan、relevance、paths)。REST 端點在 `apps/orion-chat/api/routes/memories.py`。

## 防膨脹四層設計

Memory 數量無限制成長會導致 token 成本爆、過時資訊污染回答、UI 難維護。對應四層防護:

| 層 | 解的問題 | 狀態 |
|---|---|---|
| **Layer 1**:寫入端去重 | LLM extract 重複建立相似 memory | ✅ 實作 |
| **Layer 2**:TTL / 過期淘汰 | 過時 memory 污染 prompt | ✅ 實作 |
| **Layer 3**:使用率追蹤 | 沒過期、沒重複但沒人用的「殭屍 memory」 | ⏸ 未做 |
| **Layer 4**:配額 + 建議合併 | 同類型過多、語意重複但 description 不同 | ⏸ 未做 |

四層**獨立解不同問題**,不是替代關係。Layer 1+2 處理「源頭」(寫入時 / 內容時效),3+4 處理「累積」(使用模式 / 量級爆量)。Layer 3+4 設計草案見下,未實作 — 何時做見 [`../roadmap/`](../roadmap/)。
