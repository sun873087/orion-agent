"""Phase 29:SQLite PRAGMA `foreign_keys=ON` 應 enforce user_id → users.id FK。

兩條測試:
- 寫入 user_id 是不存在的 UUID → IntegrityError(FK 真的有 enforce)
- 寫入 user_id 是存在的 users.id → 正常成功

若 FK 沒打開(Phase 29 前狀態),負向測試會錯誤 pass(write 成功)。
"""
...
from orion_agent.api.auth_db import create_user
from orion_agent.storage.db.engine import create_db_engine, db_session, init_db
from orion_agent.storage.db.models import UserSetting
