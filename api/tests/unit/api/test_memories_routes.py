...
def client_with_token(
    ...
    回 (client, token, users_root) 讓單測能 inspect fs 上實際寫了什麼。
    """
    ...
    with TestClient(create_app()) as client:
        ...
        yield client, login["token"], tmp_path / "users"
...
def test_list_empty(client_with_token: tuple[TestClient, str, Path]) -> None:
    client, token, _ = client_with_token
...
def test_delete_removes_and_updates_index(
    ...
    # MEMORY.md 不應再含 temp
    idx_path = users_root / "alice" / "memory" / "MEMORY.md"
    if idx_path.exists():
        assert "temp.md" not in idx_path.read_text()


def test_get_404(client_with_token: tuple[TestClient, str, Path]) -> None:
    client, token, _ = client_with_token
