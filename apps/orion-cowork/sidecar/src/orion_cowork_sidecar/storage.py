...
# ─── Edit snapshot metadata(Phase 31-V)──────────────────────────────


async def set_last_assistant_metadata(
    engine: AsyncEngine, session_id: str, patch: dict[str, Any],
) -> bool:
    """把 patch 合進「最近一筆 assistant role row」的 metadata_json。

    Diff viewer / undo 把 edit_snapshots 寫進 turn 末的 assistant row。merge 而非
    取代:既有 compacted_out 等 flag 不受影響。回 True 若有改 row。
    """
...
async def find_last_turn_start_index(
    engine: AsyncEngine, session_id: str,
) -> int | None:
    """找最近一輪對話的開始 message index(chronological raw row position)。

    定義「一輪」=「最近一筆 user prompt 訊息(非 tool_result)+ 之後所有
    assistant / tool messages」。undo_last_turn 用這個算 truncate 起點。
    """
    rows = await load_raw_messages(engine, session_id)
    for i in range(len(rows) - 1, -1, -1):
        role, content_json, _ = rows[i]
        if role == "user" and _is_user_prompt_row_storage(content_json):
            return i
    return None


def _is_user_prompt_row_storage(content_json: Any) -> bool:
    """role=user 但內容不全 tool_result(亦即真的 user prompt)。複製自 handlers
    避免循環 import — 同樣邏輯。"""
...
async def get_last_assistant_metadata(
    engine: AsyncEngine, session_id: str,
) -> dict[str, Any] | None:
    """讀最近一筆 assistant row 的 metadata_json。沒有回 None。"""
    async with db_session(engine) as s:
        stmt = (
            select(MessageRow.metadata_json)
            .where(
                MessageRow.session_id == session_id,
                MessageRow.role == "assistant",
            )
