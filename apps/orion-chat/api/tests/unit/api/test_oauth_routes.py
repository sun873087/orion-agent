"""/oauth/* end-to-end via dev-mock provider。
...
from orion_sdk.mcp import oauth as oauth_mod
from orion_sdk.storage.secure import EncryptedFileBackend
...
async def test_token_payload_stored_as_json(
    ...
    """
    client, token = client_with_token
    # alice 的 user_id 從 /me 拿(後是 DB row id,SecureStorage 用此索引)
