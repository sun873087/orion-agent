...
from orion_sdk.storage.paths import default_session_root
...
class DbSessionManager:
    ...
    async def get(self, user_id: str, session_id: UUID) -> Conversation | None:
        ...
        if row is None:
            return None
        try:
            provider = get_provider(row.provider, row.model)
        except ValueError:
            logger.warning(
                "session %s row has unknown provider %r — cannot recover",
    ...
    async def delete(self, user_id: str, session_id: UUID) -> bool:
        ...
        from orion_chat_api.session_manager import _rmtree_session_dir
